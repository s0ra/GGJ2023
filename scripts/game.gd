...
func _physics_process(delta):
    if get_node("Player").position.y > 48.0 * 48.0 * 30.0:
        all_cleared = true
...
#                get_node("TileMap").set_cell(x + 5, y + int(float(ceil(get_node("Player").position.y / (24.0 * 48.0)) * 24.0) - 96), -1)
        ...
        var scene = load("res://scenes/rooms/room" + str(randi() % 30 + 1) + ".tscn").instance()
        ...
        var scene2 = load("res://scenes/rooms/room" + str(randi() % 30 + 1) + ".tscn").instance()
